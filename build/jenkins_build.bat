...
copy /Y %CD%\install_client.bat %INSTALLDIR%
if %errorlevel% neq 0 (
    @echo Epics utils copy failed
    exit /b %errorlevel%
)
...
%ZIPEXE% a installer%BUILD_NUMBER%.7z %INSTALLDIR%\*

REM Add the install_client.bat to the archive
%ZIPEXE% a installer%BUILD_NUMBER%.7z install_client.bat

REM Build the installer
copy /b 7zSD.sfx + installer_config.txt + installer%BUILD_NUMBER%.7z ClientInstaller%BUILD_NUMBER%.exe
if %errorlevel% neq 0 (
...
del installer%BUILD_NUMBER%.7z
del ClientInstaller%BUILD_NUMBER%.exe
