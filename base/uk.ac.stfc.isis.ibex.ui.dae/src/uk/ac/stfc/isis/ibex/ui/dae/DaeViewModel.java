...
public class DaeViewModel extends Closer {
	...
	private final RunSummaryViewModel runSummary = registerForClose(new RunSummaryViewModel());
	private final ExperimentSetupViewModel experimentSetup = new ExperimentSetupViewModel();
	private final RunInformationViewModel runInformation = registerForClose(new RunInformationViewModel(Dae.getInstance().observables()));
	private final DetectorDiagnosticsViewModel detectorDiagnosticsViewModel = new DetectorDiagnosticsViewModel(Utils.getNewDatabindingContext());
