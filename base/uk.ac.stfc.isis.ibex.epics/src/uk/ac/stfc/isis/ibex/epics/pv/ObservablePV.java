...
public abstract class ObservablePV<T extends VType> extends ClosableObservable<T> implements PV<T> {
	...
	private static final Logger LOG = IsisLog.getLogger(ObservablePV.class);
	...
	public ObservablePV(PVInfo<T> info) {
        if (info.address().contains("BLOCKSERVER:IOCS")) {
        	LoggerUtils.logIfExtraDebug(LOG, "(Ticket 2161) ObservablePV initialized looking at " + info.address());
        }
		this.info = info;
	}
